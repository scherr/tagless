...
class Printer implements Symantics<Pri> {
    private int id = 0;
    ...
    public <A, B> HiRepr<Pri, Function<A, B>> fix(Function<HiRepr<Pri, Function<A, B>>, HiRepr<Pri, Function<A, B>>> f) {
        String sym = gensym();
        return new Pri<>( "[θ λ" + sym + "." + f.apply(new Pri<>(sym)) + "]" );
    }
